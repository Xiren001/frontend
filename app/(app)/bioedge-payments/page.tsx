...
type ProductStatus = 'done' | 'ready' | 'needs_links' | 'needs_language' | 'active'
...
const STATUS_CLS: Record<ProductStatus, string> = {
  ...
  needs_language: 'bg-amber-500/15 text-amber-600 border-amber-500/20',
  active:         'bg-zinc-500/15 text-zinc-500 border-zinc-500/20',
}

export default function BioedgePaymentsPage() {
  const { role } = useRole()
  ...
  const load = useCallback(() => {
    api.get<PaymentItem[]>('/api/bioedge-proof-corrections/payment-overview')
      .then(setItems).catch(console.error)
  }, [])

  useRealtimeRefresh('bioedge_proof_products', load)
  ...
  async function togglePaid(item: PaymentItem) {
    ...
    try {
      ...
      await api.post('/api/bioedge-proof-corrections/mark-paid', {
        id:      item.id,
        paid:    !item.paid,
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="shrink-0">
        <PageHeader
          title="BioEdge Payments"
          description="Track payment status for all BioEdge proofread products"
        />
          ...
          {visible.length === 0 ? (
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm mb-3">
                <div>
                  <p className="text-xs text-text-muted mb-0.5">Added</p>
                  <p className="font-mono text-text-secondary text-sm">
                    {item.created_at ? formatDate(item.created_at) : '—'}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-text-muted mb-0.5">{item.paid ? 'Paid date' : 'Completed'}</p>
                  <p className="font-mono text-text-secondary text-sm">
